...
def focus_decay(data, threshold):
    ...
    for t in range(1,T+1):
        ...
        M_best = 0
        for m in range(M):
            best_glr_pos, v_pos = find_best_glr(N[m], S[m], quadratics_positive[m,:quad_len_pos[m]])
            best_glr_neg, v_neg = find_best_glr(N[m], S[m], quadratics_negative[m,:quad_len_neg[m]])
            if best_glr_pos > best_glr_neg:
                if best_glr_pos > best_glr:
                    best_glr = best_glr_pos
                    v_best = v_pos
                    M_best = m
            else:
                if best_glr_neg > best_glr:
...
@njit
def change_point_estimation(data):
    # Implementation of multi-stream FOCuS for any mu
    # Each stream gets its own set of quadratics
    # Initialization
    M = data.shape[0]
    ...
    M_previous = 0
    quadratic_add = np.empty(4, dtype=np.float64)
    past_changepoints = np.zeros(T)
    selections = np.zeros(T)
    # tau, s, l, nu
    ...
    for t in range(1,T+1):
        ...
        #Okay here's a quick mistake I noticed, I'm using A_t to do the changepoint estimation when it should be whatever the best stream is
        # Remember that A_t can be picked randomly, so it's not necessarily the best
        past_changepoints[t-1] = v_previous
    return past_changepoints, selections
